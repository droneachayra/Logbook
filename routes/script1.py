...
drone1 = data['UIN'][0]
wb.create_sheet(index = 3, title = drone1)
ws4 = wb[drone1]
ws4.append(drone_log_header)
drone1_day1 = []
drone1_day2 = []

if(len(data["UIN"]) > 1):
    drone2 = data["UIN"][1]
    wb.create_sheet(index = 4, title = drone2)
    ws5 = wb[drone2]
...
if(len(data["UIN"]) == 3):
    drone3 = data["UIN"][2]
    wb.create_sheet(index = 5, title = drone3)
    ws6 = wb[drone3]
...
for student in range(count_of_trainees):

    if(temp_count == temprary):
        drone_index += 1
        temp_count = 0

    if(count_start_day1 == temprary):
        count_start_day1 = 0
    ...
    for y in range(len(ex_day1)):
        ...
        rpic = ""
        ws.append([day1, uin, clas, category, trer, trainee, place, start, end, duration, dual, rpic, cat1, ex])
...
for student in range(len(trainees)):
    ...
    values = []
    trainee = trainees[student]
    ws3.append([day3, exam_uin, clas, category, examiner, trainee, place, start, end, duration, dual, rpic, cat1, ex])
    ws3.append(values)

    try:
        if(drone1 == exam_uin):
            # ws5.append([slno, day3, trainee, place, start, end, duration, cumulative_Hrs, remarks])
            drone1_day2.append([slno, date3, trainee, place, start, end, duration, cumulative_Hrs, remarks])
        elif(drone2 == exam_uin):
            drone2_day2.append([slno, date3, trainee, place, start, end, duration, cumulative_Hrs, remarks])
        else:
            drone3_day2.append([slno, date3, trainee, place, start, end, duration, cumulative_Hrs, remarks])
